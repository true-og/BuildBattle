...
public final class ThemeArgument {
    ...
    private String printGameTypes() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            stringBuilder.append(
                    ThemeManager.GameThemes.VALUES[i].strip.toLowerCase(Locale.ENGLISH) + (i + 1 < length ? ", " : ""));
    ...
    public ThemeArgument(ArgumentsRegistry registry) {
        ...
        registry.mapArgument("buildbattleadmin", new LabeledCommandArgument("theme", "buildbattle.admin.theme",
                        ...
                        "&7Theme commands to add or remove themes for the specified game type\n&6Permission: &7buildbattle.admin.theme"))
        {
            ...
            public void execute(CommandSender sender, String[] args) {
                ...
                if (args.length < 4) {
                    ...
                    new MessageBuilder("COMMANDS_WRONG_USAGE").asKey().value(
                            "/" + registry.getPlugin().getCommandAdminPrefix() + " &c[add/remove] [gameType] [theme]")
                            .send(sender);
                    return;
                ...
                if (theme == null) {
                    ...
                    new MessageBuilder("&cThere is no game type with this name. Possible types: " + printGameTypes())
                            .prefix().send(sender);
                    return;
