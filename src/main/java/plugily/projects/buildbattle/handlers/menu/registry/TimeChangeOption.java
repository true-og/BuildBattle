...
public class TimeChangeOption {
    ...
    public TimeChangeOption(OptionsRegistry registry) {
        ...
        registry.registerOption(new MenuOption(30, "TIME",
                new ItemBuilder(XMaterial.CLOCK.parseItem())
                        .name(new MessageBuilder("MENU_OPTION_CONTENT_TIME_ITEM_NAME").asKey().build())
                        .lore(new MessageBuilder("MENU_OPTION_CONTENT_TIME_ITEM_LORE").asKey().build()).build(),
                new MessageBuilder("MENU_OPTION_CONTENT_TIME_INVENTORY").asKey().build())
        {
            ...
            public void onClick(InventoryClickEvent event) {
                ...
                // gui.addItem(new ItemBuilder(XMaterial.CLOCK.parseItem()).name(new
                // MessageBuilder("MENU_OPTION_CONTENT_TIME_TYPE_WORLD").asKey().build()).lore(plot.getTime().name()).build());
            ...
            private void addClockItem(NormalFastInv gui, Plot plot, String messageKey) {
                ...
                gui.addItem(new SimpleClickableItem(new ItemBuilder(XMaterial.CLOCK.parseItem())
                        .name(new MessageBuilder(messageKey).asKey().build()).build(), event ->
                {
                            ...
                            for (Player p : plot.getMembers()) {
                                ...
                                new MessageBuilder("MENU_OPTION_CONTENT_TIME_CHANGED").asKey().player(p)
                                        .value(new MessageBuilder(messageKey).asKey().build()).sendPlayer();
