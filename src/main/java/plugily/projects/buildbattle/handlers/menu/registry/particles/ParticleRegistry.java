...
public class ParticleRegistry {
    ...
    private final List<String> blackListedParticles = Arrays.asList("BLOCK_CRACK", "ITEM_CRACK", "ITEM_TAKE",
            "BLOCK_DUST", "MOB_APPEARANCE", "FOOTSTEP", "REDSTONE");
    ...
    private void registerParticles() {
        ...
        for (String particle : VersionUtils.PARTICLE_VALUES) {
            ...
            particleItem.setItemStack(new ItemBuilder(XMaterial
                    .matchXMaterial(
                            config.getString(particle + ".material-name", "bedrock").toUpperCase(Locale.ENGLISH))
                    .orElse(XMaterial.PAPER).parseItem())
                    .name(new MessageBuilder(config.getString(particle + ".displayname")).build()).lore(lore).build());
    ...
    private void updateParticlesFile() {
        ...
        for (String particle : VersionUtils.PARTICLE_VALUES) {
            ...
            if (!config.isSet(particle + ".material-name")) {
                ...
                config.set(particle + ".material-name", Material.PAPER.name());
                plugin.getDebugger()
                        .debug("Found outdated item in particles.yml! We've converted it to the newest version!");
    ...
    private void registerInventory() {
        ...
        PaginatedFastInv gui = new PaginatedFastInv(54,
                new MessageBuilder("MENU_OPTION_CONTENT_PARTICLE_INVENTORY").asKey().build());

        new InventoryScheme().mask(" 1111111 ").mask(" 1111111 ").mask(" 1111111 ").mask(" 1111111 ")
                .bindPagination('1').apply(gui);

        gui.previousPageItem(45,
                p -> new ItemBuilder(XMaterial.ARROW.parseItem()).name("<- " + p + "/" + gui.lastPage()).build());
        gui.nextPageItem(53,
                p -> new ItemBuilder(XMaterial.ARROW.parseItem()).name(p + "/" + gui.lastPage() + " ->").build());
        ...
        gui.setItem(49, new ItemBuilder(new ItemStack(Material.REDSTONE_BLOCK))
                ...
                .lore(Collections.singletonList(
                        new MessageBuilder("MENU_OPTION_CONTENT_PARTICLE_ITEM_REMOVE_LORE").asKey().build()))
                .build(), event ->
        {
