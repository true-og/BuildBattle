...
public class OptionsRegistry {
     ...
     * @throws IllegalArgumentException if option slot is same as one of already
     *                                  registered ones or ID of option is same as
     *                                  one of registered one
     */
    ...
    public NormalFastInv getOptionsGui() {
        ...
        if (optionsGui != null)
            return optionsGui;
        NormalFastInv gui = new NormalFastInv(inventorySize,
                new MessageBuilder("MENU_OPTION_INVENTORY").asKey().build());
    ...
    public void addGoBackItem(PaginatedFastInv gui, int slot) {
        ...
        gui.setItem(slot,
                new ItemBuilder(XMaterial.STONE_BUTTON.parseItem())
                        .name(new MessageBuilder("MENU_BUTTONS_BACK_ITEM_NAME").asKey().build())
                        .lore(new MessageBuilder("MENU_BUTTONS_BACK_ITEM_LORE").asKey().build()).build(),
                event ->
                {
